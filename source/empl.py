...
"""module to house the employee class which 
can track the dates that employees have arrived
...
class Employee:
    ...
    def __init__(self, name, age, job):
        ...
        self.job = job
        self.daysnumber = 0
        self.date = Employee.time.date()
        self.time = Employee.time.time()
        self.checkintime = Employee.time
        self.checkouttime = Employee.time
        self.totaltime = self.checkouttime - self.checkintime

        self.checkedin = False
        self.checkedout = True

        self.folder = r"employees/" + self.name.split(" ")[0].lower()  # fix the conflicting name issue
        self.savename = self.folder+"/save.bin"
        self.excelname = self.folder + "/excel.xlsx"  # donot forget to rename the excel file

        # these are employee detail , which are static
